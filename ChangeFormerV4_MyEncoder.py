...
from ChangeFormer.help_funcs import TwoLayerConv2d  #, save_to_mat
import torch.nn.functional as F
...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=2, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], device=None):
        super().__init__()
        self.num_classes = num_classes
        self.depths = depths

        # patch embedding definitions
        self.patch_embed1 = OverlapPatchEmbed(img_size=img_size, patch_size=7, stride=4, in_chans=in_chans,                ## 3->64
                                              embed_dim=embed_dims[0], device=device)
        self.patch_embed2 = OverlapPatchEmbed(img_size=img_size // 4, patch_size=3, stride=2, in_chans=embed_dims[0],      ## 64->128
                                              embed_dim=embed_dims[1], device=device)
        self.patch_embed3 = OverlapPatchEmbed(img_size=img_size // 8, patch_size=3, stride=2, in_chans=embed_dims[1],      ## 128->256
                                              embed_dim=embed_dims[2], device=device)
        self.patch_embed4 = OverlapPatchEmbed(img_size=img_size // 16, patch_size=3, stride=2, in_chans=embed_dims[2],     ## 256->512
                                              embed_dim=embed_dims[3], device=device)
        ...
        self.block1 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[0], device=device)
            for i in range(depths[0])])
        self.norm1 = norm_layer(embed_dims[0], device=device)
        # intra-patch encoder
        self.patch_block1 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[0], device=device)
            for i in range(1)])
        self.pnorm1 = norm_layer(embed_dims[1], device=device)
        # main  encoder
        cur += depths[0]
        self.block2 = nn.ModuleList([Block(
            dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[1], device=device)
            for i in range(depths[1])])
        self.norm2 = norm_layer(embed_dims[1], device=device)
        # intra-patch encoder
        self.patch_block2 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[1], device=device)
            for i in range(1)])
        self.pnorm2 = norm_layer(embed_dims[2], device=device)
        # main  encoder
        cur += depths[1]
        self.block3 = nn.ModuleList([Block(
            dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[2], device=device)
            for i in range(depths[2])])
        self.norm3 = norm_layer(embed_dims[2], device=device)
        # intra-patch encoder
        self.patch_block3 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[2], device=device)
            for i in range(1)])
        self.pnorm3 = norm_layer(embed_dims[3], device=device)
        # main  encoder
        cur += depths[2]
        self.block4 = nn.ModuleList([Block(
            dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[3], device=device)
            for i in range(depths[3])])
        self.norm4 = norm_layer(embed_dims[3], device=device)
    ...
    def forward_features(self, x):
        ...
        # print ("x1 shape after patch_embed1:              ",x1.shape, H1, W1)                     # [1, 4096, 64]   64  64

        for i, blk in enumerate(self.block1):
            x1 = blk(x1, H1, W1)
        # print ("x1 shape after block1:              ",x1.shape)                      # 1, 4096, 64
        x1 = self.norm1(x1)
        x1 = x1.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()
        # print ("x1 shape after norm1 and reshape:              ",x1.shape)           # 1, 64, 64, 64

        outs.append(x1)

        # stage 2
        x1, H1, W1 = self.patch_embed2(x1)
        # print ("x1 shape after patch_embed2:              ",x1.shape, H1, W1)                     # [1, 1024, 128])  32  32   = [1, 1024, 128]) 32 32
        ...
        # print ("x1 shape after patch_embed3:              ",x1.shape, H1, W1)                     # [1, 256, 256]) 16 16
        ...
        # print ("x1 shape after patch_embed4:              ",x1.shape, H1, W1)                     # [1, 64, 512]) 8 8
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768, device=None):
        super().__init__()
        ...
        self.H, self.W = img_size[0] // patch_size[0], img_size[1] // patch_size[1]         # 224//7 = 32
        self.num_patches = self.H * self.W                                                  # 32*32 = 1024
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride,     # 3, 768, kernel_size=7, stride=4
                              padding=(patch_size[0] // 2, patch_size[1] // 2), device=device)               # 3, 3
    ...
    def forward(self, x):                             # input x: 1, 3, 256, 256
        # pdb.set_trace()
        x = self.proj(x)
        # print ("x after proj shape",x.shape)              # 1, 768, 64, 64
        _, _, H, W = x.shape
        x = x.flatten(2)
        # print ("x after flatten shape",x.shape)           # 1, 768, 4096
        x = x.transpose(1, 2)
        # print ("x after transpose shape",x.shape)           # 1, 4096, 768
        x = self.norm(x)
        # print ("x after norm shape",x.shape)           # 1, 4096, 768

        return x, H, W                                # [1, 4096, 768]  , H=64 , W=64
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, device=None):
        super().__init__()
        ...
        if sr_ratio > 1:
            self.sr = nn.Conv2d(dim, dim, kernel_size=sr_ratio, stride=sr_ratio, device=device)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, device=None):
        super().__init__()
        self.norm1 = norm_layer(dim, device=device)
        self.attn = Attention(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio, device=device)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim, device=device)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, device=device)
...
class DWConv(nn.Module):
    def __init__(self, dim=768, device=None):
        super(DWConv, self).__init__()
        self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim, device=device)
...
class Tenc(EncoderTransformer):
    def __init__(self, device=None, **kwargs):
        super(Tenc, self).__init__(
            patch_size=16, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, device=None)
...
class convprojection_base(nn.Module):
    def __init__(self, path=None, device=None, **kwargs):
        super(convprojection_base,self).__init__()

        # self.convd32x = UpsampleConvLayer(512, 512, kernel_size=4, stride=2)
        self.convd16x = UpsampleConvLayer(512, 256, kernel_size=4, stride=2, device=device)
        self.dense_4 = nn.Sequential(ResidualBlock(256, device=device))
        self.convd8x = UpsampleConvLayer(256, 128, kernel_size=4, stride=2, device=device)
        self.dense_3 = nn.Sequential(ResidualBlock(128, device=device))
        self.convd4x = UpsampleConvLayer(128, 64, kernel_size=4, stride=2, device=device)
        self.dense_2 = nn.Sequential(ResidualBlock(64, device=device))
        self.convd2x = UpsampleConvLayer(64, 16, kernel_size=4, stride=2, device=device)
        self.dense_1 = nn.Sequential( ResidualBlock(16, device=device))
        self.convd1x = UpsampleConvLayer(16, 8, kernel_size=4, stride=2, device=device)
...
#         res16x = res32x + x1[3]
        ...
        # print ("################ res16x shape after convd16x:         ",res16x.shape)
        ...
        res8x = self.dense_4(res16x) + x1[2]
        # print ("res8x shape after dense_4:         ",res8x.shape)
        res8x = self.convd8x(res8x) 
        # print ("###################### res8x shape after convd8x:         ",res8x.shape)
        res4x = self.dense_3(res8x) + x1[1]
        # print ("res4x shape after dense_3:         ",res4x.shape)
        res4x = self.convd4x(res4x)
        # print ("##################### res4x shape after convd4x:         ",res4x.shape)
        res2x = self.dense_2(res4x) + x1[0]
        # print ("res2x shape after dense_2:         ",res2x.shape)
        res2x = self.convd2x(res2x)
        # print ("##################### res2x shape after convd2x:         ",res2x.shape)
        x = res2x
        # print ("x shape before dense_1:             ",x.shape)
        x = self.dense_1(x)
        # print ("x shape after dense_1:              ",x.shape)
        x = self.convd1x(x)
        return x                                     # [1, 8, 256, 256]

### This is the basic ChangeFormer module
class ChangeFormerV1(nn.Module):

    def __init__(self, input_nc=3, output_nc=1, decoder_softmax=False, device=None):     # output_nc=2
        super(ChangeFormerV1, self).__init__()

        self.Tenc               = Tenc(device=device)
        
        self.convproj           = convprojection_base(device=device)

        self.change_probability = ConvLayer(8, output_nc, kernel_size=3, stride=1, padding=1, device=device)
    ...
    def forward(self, x1, x2):
        ...
        fx1 = self.Tenc(x1)
        fx2 = self.Tenc(x2)
        # print ("fx1 shape:              ",fx1[0].shape, fx1[1].shape, fx1[2].shape, fx1[3].shape)   # [1, 64, 64, 64]  [1, 128, 32, 32]  [1, 256, 16, 16]  [1, 512, 8, 8]
...
class MLP(nn.Module):
    ...
    Linear Embedding
    """
    def __init__(self, input_dim=2048, embed_dim=768, device=None):
        super().__init__()
...
class TDec(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                    ...
                    decoder_softmax = False, feature_strides=[4, 8, 16, 32], device=None):
        ...
        # print ("c1_in_channels shape is:    ",c1_in_channels) #64
        # print ("c2_in_channels shape is:    ",c2_in_channels) #128
        # print ("c3_in_channels shape is:    ",c3_in_channels) # 320
        # print ("c4_in_channels shape is:    ",c4_in_channels) #512

        self.linear_c4 = MLP(input_dim=c4_in_channels, embed_dim=self.embedding_dim,  device=device)
        self.linear_c3 = MLP(input_dim=c3_in_channels, embed_dim=self.embedding_dim,  device=device)
        self.linear_c2 = MLP(input_dim=c2_in_channels, embed_dim=self.embedding_dim,  device=device)
        self.linear_c1 = MLP(input_dim=c1_in_channels, embed_dim=self.embedding_dim,  device=device)
        ...
        self.convd2x    = UpsampleConvLayer(self.embedding_dim, self.embedding_dim, kernel_size=4, stride=2, device=device)
        self.dense_2x   = nn.Sequential( ResidualBlock(self.embedding_dim, device=device))
        self.convd1x    = UpsampleConvLayer(self.embedding_dim, self.embedding_dim, kernel_size=4, stride=2, device=device)
        self.dense_1x   = nn.Sequential( ResidualBlock(self.embedding_dim, device=device))

        #Final prediction
        self.change_probability = ConvLayer(self.embedding_dim, self.output_nc, kernel_size=3, stride=1, padding=1, device=device)
        self.output_softmax     = decoder_softmax
...
class ChangeFormerV2(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=1, decoder_softmax=False, device=None):
        ...
        self.TDec   = TDec( input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                            ...
                            decoder_softmax = decoder_softmax, feature_strides=[4, 8, 16, 32], device=device)
        #Final activation
...
class ChangeFormerV3(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=1, decoder_softmax=False, device=None):
        ...
        #Transformer Encoder
        self.Tenc   = Tenc( patch_size=16, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], 
                            mlp_ratios=[4, 4, 4, 4], qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), 
                            depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, device=device)
        
        #Transformer Decoder
        self.TDec   = TDecV2( input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                            in_channels = [64, 128, 256, 512], embedding_dim= 64, output_nc=output_nc, 
                            decoder_softmax = False, feature_strides=[4, 8, 16, 32], device=device)
...
class EncoderTransformer_x2(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=3, num_classes=2, embed_dims=[32, 64, 128, 256, 512],
                 ...
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 3, 6, 18, 3], sr_ratios=[8, 4, 2, 1, 1], device=None):
        super().__init__()
        self.num_classes    = num_classes
        self.depths         = depths
        ...
        self.patch_embed5 = OverlapPatchEmbed(img_size=img_size // 16, patch_size=3, stride=2, in_chans=embed_dims[3],
                                              embed_dim=embed_dims[4], device=device)
       ...
       # Stage-3 (x1/8 scale)
        ...
        self.block5 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[4], device=device)
            for i in range(depths[4])])
        self.norm5 = norm_layer(embed_dims[4], device=device)
...
def conv_diff(in_channels, out_channels, device=None):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, device=device),
        nn.ReLU(),
        nn.BatchNorm2d(out_channels, device=device),
        nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, device=device),
        nn.ReLU()
    )

def make_prediction(in_channels, out_channels, device=None):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1, device=device),
        nn.ReLU(),
        nn.BatchNorm2d(out_channels, device=device),
...
class DecoderTransformer_x2(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3, 4], align_corners=True, 
                    ...
                    decoder_softmax = False, feature_strides=[2, 4, 8, 16, 32], device=None):
        ...
        self.linear_c5 = MLP(input_dim=c5_in_channels, embed_dim=self.embedding_dim , device=device)
        self.linear_c4 = MLP(input_dim=c4_in_channels, embed_dim=self.embedding_dim, device=device)
        self.linear_c3 = MLP(input_dim=c3_in_channels, embed_dim=self.embedding_dim, device=device)
        self.linear_c2 = MLP(input_dim=c2_in_channels, embed_dim=self.embedding_dim, device=device)
        self.linear_c1 = MLP(input_dim=c1_in_channels, embed_dim=self.embedding_dim, device=device)
        ...
        self.diff_c5   = conv_diff(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim, device=device)
        self.diff_c4   = conv_diff(in_channels=3*self.embedding_dim, out_channels=self.embedding_dim, device=device)
        self.diff_c3   = conv_diff(in_channels=3*self.embedding_dim, out_channels=self.embedding_dim, device=device)
        self.diff_c2   = conv_diff(in_channels=3*self.embedding_dim, out_channels=self.embedding_dim, device=device)
        self.diff_c1   = conv_diff(in_channels=3*self.embedding_dim, out_channels=self.embedding_dim, device=device)

        #Taking outputs from middle of the encoder
        self.make_pred_c5 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc, device=device)
        self.make_pred_c4 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc, device=device)
        self.make_pred_c3 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc, device=device)
        self.make_pred_c2 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc, device=device)
        self.make_pred_c1 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc, device=device)
...
class ChangeFormerV4(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=1, decoder_softmax=False):
        super(ChangeFormerV4, self).__init__()
        #Transformer Encoder
        self.embed_dims = [32, 64, 128, 256, 512]
        ...
        self.TDec_x2   = DecoderTransformer_x2(input_transform='multiple_select', in_index=[0, 1, 2, 3, 4], align_corners=True, 
                    ...
                    decoder_softmax = None, feature_strides=[2, 4, 8, 16, 32])
...
########### with Hazem changes from EncoderTransformer_x2 ##############
class EncoderTransformer_x3(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=3, num_classes=2, embed_dims=[32, 64, 128, 256, 512],
                 num_heads=[2, 2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3, 3], sr_ratios=[8, 4, 2, 1, 1], device=None):     #  ORIGINAL CHANGEFORMERV4 ENCODER  depths=[3, 3, 6, 18, 3]
        super().__init__()
        self.num_classes    = num_classes
        self.depths         = depths
        self.embed_dims     = embed_dims
...
if __name__ == "__main__":
    ...
    # y = model(x)
    # a, H, W = y
    # print(a.shape, H, W)   # [1, 4096, 768]  64  64

    ####### Test OverlapPatchEmbed, ########## patch_embed2 ########
    # x = torch.randn(1, 64, 64, 64)
    # print ("original shape ",x.shape)
    # model = OverlapPatchEmbed(img_size=64,patch_size=3, stride=2, in_chans=64, embed_dim=128)   # patch_embed2
    # y = model(x)
    # a, H, W = y
    # print(a.shape, H, W)             # [1, 1024, 128]) 32 32        ->> same output as in the EncoderTransformer above


    ###### Test Attention ######
    # x = torch.randn(1, 4096, 768)
    # H, W = 64, 64
    # print ("original shape:           ",x.shape)           # [1, 4096, 768]
    # model = Attention(dim=768, num_heads=8)
    # y = model(x, H, W)
    # print("Attention output shape:        " , y.shape)     # [1, 4096, 768]
    ...
    # y = model(x, H, W)
    # print("Block output shape:        " , y.shape)         # [1, 4096, 768]



    ####### Test Conv2d with stride=4 ######
    # x = torch.randn(1, 3, 256, 256)
    # print ("original shape ",x.shape)
    # m = nn.Conv2d(3, 768, kernel_size=7, stride=4, padding=3)               
    # y = m(x)
    # print(y.shape)  # 1, 768, 64, 64


    # ####### EncoderTransformer - Tenc ########
    # x = torch.randn(1, 3, 256, 256)
    # print ("original shape ",x.shape)
    # model = EncoderTransformer()
    # y = model(x)
    # print(y[0].shape, y[1].shape, y[2].shape, y[3].shape)                # [1, 64, 64, 64]  [1, 128, 32, 32]  [1, 256, 16, 16]  [1, 512, 8, 8]
    ...
    # x = [x1, x2, x3, x4]
    # print ("original shape ",x1.shape, x2.shape, x3.shape, x4.shape)
    # model = convprojection_base()
    # y = model(x)
    ...
    # x1 = torch.randn(1, 3, 256, 256)
    # x2 = torch.randn(1, 3, 256, 256)
    # model = ChangeFormerV1()  #decoder_softmax=True)
    ...
    # x1 = torch.randn(1, 3, 256, 256)
    # x2 = torch.randn(1, 3, 256, 256)
    # model = ChangeFormerV2()  #decoder_softmax=True)
    ...
    # x1 = torch.randn(1, 3, 256, 256)
    # x2 = torch.randn(1, 3, 256, 256)
    # model = ChangeFormerV3()  #decoder_softmax=True)
    ...
    # x1 = torch.randn(1, 3, 256, 256)
    # x2 = torch.randn(1, 3, 256, 256)
    # model = ChangeFormerV4()  #decoder_softmax=True)
    
    # y = model(x1, x2)
    # print(y[0].shape, y[1].shape, y[2].shape, y[3].shape, y[4].shape, y[5].shape)
    # print (len(y))
    # print (y)
    # print ("Max value: ",torch.max(y[5]), "Min value: ", torch.min(y[5]))
    ...
    x1 = torch.randn(1, 3, 256, 256)
    model = EncoderTransformer_x3()  #decoder_softmax=True)
